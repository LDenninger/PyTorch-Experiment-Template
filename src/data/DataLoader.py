"""
    A custom data loader for the Cityscapes dataset adapted from: https://github.com/meetshah1995/pytorch-semseg/blob/master/ptsemseg/loader/cityscapes_loader.py
...
class CityScapesLoader(data.Dataset):
    ...
    def __init__(
        ...
        # Resizing operations
        self.img_resize = v2.Resize(self.img_size, interpolation=v2.InterpolationMode.BILINEAR, antialias=True)
        self.seg_resize = v2.Resize(self.img_size, interpolation=v2.InterpolationMode.NEAREST, antialias=True)
    ...
    def __getitem__(self, index):
        ...
        img = torch.from_numpy(img).float().permute(2, 0, 1).to(self.device)  # HWC to CHW
        lbl = torch.from_numpy(lbl).float().unsqueeze(0).to(self.device)

        if self.is_transform:
            img = self.img_resize(img)
            lbl = self.seg_resize(lbl)
